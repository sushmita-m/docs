...
1) If a repository or owner is not passed after the subscribe command, the error message, _Please add repository or organization URL after the subscribe command._ is shown e.g., `@GitHub subscribe`
...
3) If the user is signed-in, we are going to check if the GitHub resource (owner or owner/repository) passed in the command are in a valid format and that if the account exists on GitHub.
   * Firstly, if a URL is passed in the command, we verify the validity of the URL using the [npm url module](https://www.npmjs.com/package/url). If the owner or owner/repository could not be extracted from the URL or if the URL passed is not a GitHub URL, the error message: _url does not appear to be a GitHub repository or organization link._, will be shown. We are not going to process a http: protocol, the same error message will be shown in this case too.
   * if the arguments passed doesn't match the owner or owner/repository format, the error message, _url does not appear to be a GitHub repository or organization link._ will be shown.
   * Once this check is done, we are going to check if the user exist (could be an account name or an organization name) [GetByUserName](https://octokit.github.io/rest.js/v17#users-get-by-username).
   * If the user could not be found, the error message, _Could not find resource_ will be shown.
   * If a repo is also passed, we use [Get](https://octokit.github.io/rest.js/v17#repos-get) to check the validity of the repository. This API uses the user's identity to check if the user can access the repository.
   * If an installation for the Teams app is not found on the repository, _install app_ card is shown.
   * If an installation is found, it indicates either the repository does not exist or that the user cannot access the repository. The error message, _Could not find resource_ will be shown.
...
1) If a repository or owner is not passed after the unsubscribe command, the error message, _Please add repository or organization URL after the subscribe command._ is shown e.g., `@GitHub unsubscribe`
2) Then we check if the user is signed-in, if the user is not signed in, user will be prompted with a sign in card.
3) If the user is signed-in, we are going to check if the GitHub resource (owner or owner/repository) passed in the command is in valid format and that if the account exists on GitHub.
   * Firstly, if a URL is passed in the command, we verify the validity of the URL using the [npm url module](https://www.npmjs.com/package/url). If the owner or owner/repository could not be extracted from the URL or if the URL passed is not a GitHub URL, the error message: _url does not appear to be a GitHub repository or organization link._, will be shown. We are not going to process a http: protocol, the same error message will be shown in this case too.
   * if the arguments passed doesn't match the owner or owner/repository format, the error message, _url does not appear to be a GitHub repository or organization link._ will be shown.
   * Once this check is done, we are going to use to check if an user exists (could be an account name or an organization name) [GetByUserName](https://octokit.github.io/rest.js/v17#users-get-by-username)
   * If the user could not be found, the error message, _Could not find resource_ will be shown
   * If a repo is also passed, we use [Get](https://octokit.github.io/rest.js/v17#repos-get) to check the validity of the repository. This API uses the user's identity to check if the user can access the repository. If the repository cannot be accessed or does not exist, install app card is shown.
